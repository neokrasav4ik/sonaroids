...
**Two ways to play (v0.35):** on the table, or the phone held in the other hand with the palm edge-on at the port end, 5–15 cm away (the maintainer, 26 Sep: as precise as on the table, closer means lower; 5–10 cm until v0.39 — on the OnePlus it plays better a bit farther). The player chooses nothing — the sonar measures the distance to the palm either way, so the game needs no switch. The getting-ready screens show both pictures side by side with "OR" between them: on the left the table (smaller than before, with a thin edge), on the right the phone lying in the left palm (fingers over the top, thumb under) and the right palm straight, edge down, parallel to the phone's end, with its own 5–10 cm ruler. The port label sits under each phone with an arrow up. Grips tried: three sketches, the maintainer chose "lying in the palm". — agreed 26–27 Sep
...
- **Language screen (v0.31):** when the game is open in a phone's browser, not from the home screen, a line under the buttons in both languages: "For full screen mode, add the site to your home screen". Of four places tried this one was chosen (the maintainer, 25 Sep); no separate step-by-step picture screen. — decided 25 Sep
- **A covered speaker (v0.34):** when the palm or fingers cover the speaker or the microphone, the sonar changes nothing and the player sees a red line "DON'T COVER THE SPEAKER AND MIC" (under the title on the wave step, under the score in flight). Prompted by playing with the phone in the hand (26 Sep): the palm at the port end, sometimes right against it. — decided 26 Sep
- **A shifted response (v0.37):** in the hand-held games the whole response (the direct sound and the room) sometimes jumps along the delay with no input dropout — iPhone 26 Sep 10:29 by 256 samples, OnePlus 15:53 by ~144. v0.34 took it for a covered speaker: stopped the position pull, the ship drifted up, and the player saw "don't cover" with the port end free (the maintainer). Now, when the direct sound is gone, the sonar looks for a shifted copy of the whole complex response gathered over ~0.3 s (the moving palm's echo cancels out, the direct sound stays), moves there, and refines over the next 2 s. "Covered" is left only when no shifted copy is found. — 26 Sep
...
- **The place on the game-over screen (v0.32):** the place of this very game; when the player has a better game, a second line with the record's place ("this game: #2 all time" / "your best: #1"). Before, only the record's place was shown, and a 798 thousand game said "#1" without getting into the table (one line per player there — the best game). — decided 26 Sep
- **The transfer code (v0.32):** Safari, the home screen and another phone keep their own player key, so the server sees different players (the maintainer had two neokrasav4ik lines). HIGH SCORES → TRANSFER: "show a code" on one device (6 characters, 10 minutes, kept only in the server's memory), "enter a code" on the other — it takes over the first device's key, and its games, reports and name (if the first has none) join that player. Same names are not forbidden. — decided 26 Sep
