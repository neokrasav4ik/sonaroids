...
- `tools/` — инструменты разбора и проверки; все берут код из собранного приложения:
  ...
  - `eval_dual.js` — разбор записи «Два динамика» (`sonardual_*.wav`): звучат ли каналы из разных торцов и видно ли по двум расстояниям, где ладонь вдоль телефона; `--synth` — самопроверка;
  ...
  - `tests/` — стенды страницы: подготовка без калибровки, центровка, подстройка по взмахам, игра, журнал партии, журнал настройки, пропажа зонда, автоуровень, запуск микрофона, запись вбок в портрете (`test_side_rec.js`), два динамика (`test_dual_rec.js`);
