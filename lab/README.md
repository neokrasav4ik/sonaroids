...
- `tools/` — инструменты разбора и проверки; все берут код из собранного приложения:
  ...
  - `eval_right.js` — разбор пробы-игры «Ладонь справа» (`sonarright_*.wav`): видна ли ладонь, дрожь корабля, ход ладони на весь экран, сверка с телефоном; запись по метке `sonar1r_*.wav` разбирает `eval_recording.js`;
  - `eval_ark.js` — разбор записи прототипа Арканоида (`sonarark_*.wav`): ладонь, дрожь ракетки, отбито, промахи и как далеко была ракетка;
  ...
  - `tests/` — стенды страницы: подготовка без калибровки, центровка, подстройка по взмахам, игра, журнал партии, журнал настройки, пропажа зонда, автоуровень, запуск микрофона, запись вбок в портрете (`test_side_rec.js`), два динамика (`test_dual_rec.js`), ладонь справа (`test_right.js`), прототип Арканоида (`test_ark.js`);
