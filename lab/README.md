...
- `tools/` — инструменты разбора и проверки; все берут код из собранного приложения:
  ...
  - `tests/` — стенды страницы: подготовка без калибровки, центровка, подстройка по взмахам, игра, журнал партии, журнал настройки, пропажа зонда, автоуровень, запуск микрофона, запись вбок в портрете (`test_side_rec.js`), два динамика (`test_dual_rec.js`), ладонь справа (`test_right.js`);
