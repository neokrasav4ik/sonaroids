...
var DSP2=(function(){
  ...
  var eAvg=0,TE=3,ePres=0,why='',rngBuf=[],xBuf=[],centered=false,autoC=false,presN=0,x,fast,cal={k:0.75,o:1,s:0.8},eHold=null,lowN=0,resS=-99,resFloor=null,upN=0,warm=0,absBuf=[],ABS_MED=15,DEADB=5,dirS=null,lostN=0,lost=false;
  ...
  function frame(fr){
    ...
    if(present){ if(E>T_INT) x+=cal.s*vel;
      ...
      eAvg+=(1-Math.exp(-1/(TE*fpsF)))*((absM-x)-eAvg);
      var ex=Math.abs(eAvg)>DEADB?eAvg-(eAvg>0?DEADB:-DEADB):0, dx=(1-Math.exp(-1/(TAU*fpsF)))*ex;
      x+=dx; eAvg-=dx; }
