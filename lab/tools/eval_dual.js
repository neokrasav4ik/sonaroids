...
function analyse(meta,x){ const flo=C.bandOf(meta), out={};
  out.L=direct(seg(x,meta,'e_l','e_r'),flo); out.R=direct(seg(x,meta,'e_r','e_d'),flo);
...
function report(name,meta,x){ const R=analyse(meta,x);
  ...
  const apart=Math.abs(dW)>=8, cov=Math.min(...R.coverDrop)<=-4&&Math.abs(R.coverDrop[0]-R.coverDrop[1])>=3;
