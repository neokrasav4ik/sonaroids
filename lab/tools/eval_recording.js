/* Оценка механики на «Записи для меня» (сценарий: пусто 0–3 с, ставит 3–5, ведёт по синусоиде 5–11, держит 11–14, убирает 14–16).
   Запуск: node eval_recording.js [--phys] запись1.wav [запись2.wav ...]
   --phys — калибровка как в игре (PHYS_CAL приложения) вместо самокалибровки по пикам синусоиды. Самокалибровка ломается,
            если ладонь ходила не по метке 100 ± 50 мм (например, запись 11:09 с размахом 4–12 см); «по форме» от сдвига и масштаба не зависит.
   Метрики:
...
function shape(o){ const mv=o.filter(r=>r.t>=5.3&&r.t<11&&r.present), ts=mv.map(r=>r.t), hs=mv.map(r=>r.height); let best=null;
  if(mv.length<50) return null;   // руки почти не видно — сравнивать не с чем
