...
# журналы партий — game_log.py и replay_game.js; длинные записи по метке (sonarlong_*) — eval_long.js; записи вбок (sonarside_*, телефон вертикально) — eval_side.js. Журналы и лабы (sonar_*), и самой игры (sonaroids_*).
...
L=$(ls "$D"/sonarlong_*.wav 2>/dev/null); SD=$(ls "$D"/sonarside_*.wav 2>/dev/null)
