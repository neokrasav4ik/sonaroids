"""Packs promo/frames/*.png (from make_gif.js) into promo/sonaroids.gif (VARIANT=clean: frames_clean → sonaroids_clean.gif): the whole 16:9 frame, one shared palette, looping forever.
Run: python3 promo/make_gif.py [scale]   — scale < 1 makes a smaller file (nearest-neighbour, keeps the pixels crisp)."""
import glob, os, sys
from PIL import Image
HERE = os.path.dirname(os.path.abspath(__file__))
CLEAN = os.environ.get('VARIANT') == 'clean'   # VARIANT=clean: frames_clean → sonaroids_clean.gif
files = sorted(glob.glob(os.path.join(HERE, 'frames_clean' if CLEAN else 'frames', '*.png')))
...
out = os.path.join(HERE, 'sonaroids_clean.gif' if CLEAN else 'sonaroids.gif')
