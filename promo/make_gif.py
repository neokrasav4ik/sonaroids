"""Packs promo/frames_<VARIANT>/*.png (from make_gif.js) into the GIF of that variant (see make_gif.js): the whole 16:9 frame, one shared palette, looping forever.
Run: python3 promo/make_gif.py [scale]   — scale < 1 makes a smaller file (nearest-neighbour, keeps the pixels crisp)."""
import glob, os, sys
from PIL import Image
HERE = os.path.dirname(os.path.abspath(__file__))
V = os.environ.get('VARIANT', 'clean')   # the same VARIANT as for make_gif.js: frames_<V> → the GIF of that name
NAME = {'clean': 'sonaroids.gif', 'table': 'sonaroids_table.gif', 'hand': 'sonaroids_hand.gif', 'both': 'sonaroids_both_table.gif'}[V]
files = sorted(glob.glob(os.path.join(HERE, 'frames_' + V, '*.png')))
