/* The whole game in headless Chromium with a synthetic microphone (Sonar.simulate): title → take your hand away → wave →
   ...
   the setup log replays through the lab's replay_game.js; no page errors.
...
(async()=>{
  ...
  while(T()<40){
    ...
    if(s.scr==='play'){ if(s.hand!==null&&s.ship!==null) follow.push([s.hand,s.ship]); if(T()>26&&!shots.play){ shots.play=1; await shot('05_play'); } }
  ...
  const m=rep.match(/итог \|Δ\| медиана ([\d.]+) мм/);
  ...
  console.log(`logs: setup ${logs.setup?(logs.setup.length/1024).toFixed(0)+' KB':'none'}, game ${logs.game?(logs.game.length/1024).toFixed(0)+' KB':'none'}; lab replay of the setup log: final height differs from the page by ${m?m[1]:'?'} mm (median)`);
  if(errors.length) console.log('page errors:',errors.join(' | '));
  const need=['title','away','wave','count','play','over'], got=need.every(n=>seen.some(s=>s.startsWith(n+'@')));
  const ok=got&&caughtAt!==null&&range&&range[0]>0.05&&range[0]<0.16&&range[1]>0.84&&range[1]<0.95&&corr>0.95&&logs.setup&&logs.game&&m&&+m[1]<1.5&&!errors.length;
