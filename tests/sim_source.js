/* A synthetic microphone for headless tests: the game's current probe (F_LO from the game code) with a direct path, one fixed
   ...
   Used inside the page (Playwright addInitScript): defines window.__simSource(scenario) → source(i) for Sonar.simulate. */
function makeSimSource(h, opts){
  ...
  return function(i){
    ...
    for(n=0;n<N;n++){ seed=(seed*1664525+1013904223)>>>0; out[n]+=(seed/4294967296-0.5)*4e-4; }
    return out;
  };
}
