...
function decide(scale,noise){ const s=make(()=>null), fr=[]; let seed=7;
  for(let i=0;i<8;i++){ const f=s(i+20).map(v=>v*scale); for(let n=0;n<512;n++){ seed=(seed*1664525+1013904223)>>>0; f[n]+=(seed/4294967296-0.5)*noise*0.0141; } fr.push(f); }
  const st=stats(fr,48000,18300,20450), lvl=st.line-20*Math.log10(G); return {snr:st.snr,lvl,quiet:lvl<QUIET||st.snr<18}; }
const cases=[['sound on, quiet room',1,0,false],['sound on, noisy room (like 24 Sep, 33 dB)',1,0.02,false],['silent mode (probe −30 dB)',0.03,0,true],['volume near zero (−20 dB)',0.1,0,true]];
let ok=true; cases.forEach(([name,sc,nz,want])=>{ const d=decide(sc,nz); const good=d.quiet===want; ok=ok&&good;
