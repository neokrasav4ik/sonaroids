...
const BUTTONS=['play','recal','howto','lang','sfx_on','sfx_off','next','allow','start','again','menu','logs','retry','resume','quit','exit','restart','rs_go','back','scores','p_day','p_week','p_all','name_set','done','later'];
for(const l of ['en','ru']){ for(const k in STR[l]) for(const c of STR[l][k].replace('{n}','0')) if(!PF.has(c)) miss.add(c);
