...
const BUTTONS=['play','recal','howto','lang','sfx_row','sfx_off','next','allow','start','again','menu','logs','retry','resume','quit','exit','restart','rs_go','back','scores','p_day','p_week','p_all','name_set','done','later','link','link_show','link_in'];
for(const l of ['en','ru']){ for(const k in STR[l]) for(const c of STR[l][k].replace(/\{\w+\}/g,'0')) if(!PF.has(c)) miss.add(c);
